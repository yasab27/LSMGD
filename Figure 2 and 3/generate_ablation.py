...
X = pd.read_csv("./data/env_isolate_curves.csv", header = None).to_numpy()
y = pd.read_csv("./data/env_isolate_labels.csv", header = None).to_numpy().squeeze()
...
# Generate the plots
gen_plot(base_test_score, em_dims, test_scores, "Isolates", "Testing Accuracy", experiment_name)
gen_plot(base_train_score, em_dims, train_scores, "Isolates", "Training Accuracy", experiment_name)

final_data = np.array([em_dims, train_scores, test_scores]).T
final_data = np.vstack([final_data, [0, base_train_score, base_test_score] ])

np.savetxt(f"./figures/{experiment_name}/classification_results/Final Data.csv", final_data, delimiter = ",")